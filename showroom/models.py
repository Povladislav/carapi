...
class ShowRoom(IsActiveMixin, DateMixin):
    ...
    preferable_cars = models.ManyToManyField(
        "car.PreferableCar", related_name="preferable_cars", blank=True
    )
    history = models.ManyToManyField(
        "History", related_name="history_of_sells", blank=True
    )
    discount = models.ManyToManyField(
        "Discount", related_name="discount_for_customer", blank=True
    )
...
class History(IsActiveMixin, DateMixin):
    ...
    buyer_showroom = models.ForeignKey(
        "showroom.ShowRoom",
        on_delete=models.CASCADE,
    ...
    whole_price = models.DecimalField(
        max_digits=6, decimal_places=0, validators=[MinValueValidator(0)]
    )
