    ...
    for a_car in cars_for_showroom.filter(available_car=p_car.preferable_car):
        if a_car.producer.discount.exists():
            discount = Discount.objects.filter(
                Q(car=a_car.available_car) & Q(date_of_start__lt=timezone.now()) & Q(
                    date_of_end__gt=timezone.now())).first()
...
def showroom_buy_car():
    for showroom in showrooms:
        for p_car in showroom.preferable_cars.all():
            ...
            if most_benefit_car is not None and int(p_car.count) == int(most_benefit_car.count):
                ...
                history = History.objects.create(buyer_showroom=showroom, count=p_car.count,
                ...
                showroom.preferable_cars.filter(preferable_car=p_car.preferable_car).delete()
                showroom.save()
                most_benefit_car.save()
                AvailableCar.objects.create(available_car=most_benefit_car.available_car,
                                            count=p_car.count, showroom=showroom,
                ...
                AvailableCar.objects.create(available_car=most_benefit_car.available_car,
                                            count=most_benefit_car.count, showroom=showroom,
                                            price=round(float(most_benefit_car.price) / 0.7))
